...
public:
	...
	*/
	FRNG(uint64_t Seed = unsigned(time(nullptr)));

	/** 
	* Returns a random floating point from min to max inclusive 
...
{
	uint64_t Mod = (uint64_t)Max - (uint64_t)Min + 1;
